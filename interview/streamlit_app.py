...
LEADERBOARD_PATH = os.path.join(EXCEL_DIR, "all_candidates.json")
...
def interview_page():
    ...
    with col1:
        ...
        # Video processing with continuous updates
        ...
        # Auto-refresh for continuous video feed
        if not st.session_state.is_recording:  # Only refresh when not recording
            time.sleep(0.1)
            st.rerun()
    ...
    with col2:
        ...
        if not st.session_state.is_recording and not st.session_state.recording_complete:
            ...
            st.info("Recording for 15 seconds. Please speak clearly.")
